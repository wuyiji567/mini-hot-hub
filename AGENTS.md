...
> ⚠️ **状态（2026-06）**：MVP 第一阶段已完成并上线（4 平台真实数据 + 综合热榜 + 缓存降级，部署于 Vercel + Railway）。
> **当前进入第二阶段**——新增 4 平台、接入 AI、米白视觉改版。第二阶段的任务、顺序与决策见 [`docs/PHASE2_TASKS.md`](docs/PHASE2_TASKS.md)。
> 本文件下方的「不要在第一阶段实现 AI / 新平台」等约束**仅针对第一阶段**，第二阶段已解除；但类型契约、编码规范、降级规则、缓存规则等**仍然有效**，继续遵守。
