...
| 阶段 | 范围 | 说明 |
|------|------|------|
| MVP 第一阶段 | 微博、知乎、B 站、GitHub；综合热榜；平台卡片；基础缓存；错误降级 | 第一版优先完成，确保项目能跑通 |
| MVP 第二阶段 | 澎湃、36氪、虎扑、今日头条；AI 热门原因；分类标签；「汽车」独立标签 | 基础功能稳定后接入，完成 PRD 定义的 8 核心平台 |
| 未来版 | 抖音；登录态个性化 / 用户行为画像推荐；独立 AI 接口或实时推送；更复杂趋势算法 | 难度更高，避免拖慢 MVP |

说明：PRD 中定义的"MVP"对应本文的"MVP 第一阶段 + 第二阶段"。技术侧拆成两步是为了降低首次交付风险，但产品验收标准仍以 PRD 的 8 核心平台为准。

## 技术栈

- 前端：React + TypeScript + Vite + CSS Modules（深色科技风主题）
- 后端：Node.js + Express + TypeScript
- AI：DeepSeek API，可选能力；模型通过环境变量配置，按官方当前可用模型填写
- 缓存：MVP 第一阶段使用内存 Map + 完整响应缓存 300 秒；PRD MVP 验收前拆分为单平台缓存和 AI 缓存
...
- `AI_ENABLED=false` 时，后端不调用 AI，仍然返回基础热榜；本地开发可关闭，生产 MVP 第二阶段应开启。
...
7. 如果 `AI_ENABLED=true` 且 AI 服务可用，再补充 AI 标签、热门原因和推荐内容，并返回 `ai.status="ready"`。
8. 如果基础热榜已就绪但 AI 仍在生成，先返回 `ai.available=false` + `ai.status="pending"` + 基础数据，AI 结果在后台写入缓存。
9. 如果 AI 超时、调用失败或格式异常，返回 `ai.available=false` + `ai.status="unavailable"`，基础热榜正常返回。
10. 最终响应写入缓存，前端渲染首页和平台页。

MVP 不新增独立 `/api/ai` 接口。AI 补齐使用 `/api/hot` 的 `ai.status` 表达：前端首次加载遇到 `pending` 时，30 秒后自动重新请求一次 `/api/hot`，只重试一次、不轮询。
...
如果 `ai.featured` 和 `ai.recommendations` 为空，前端根据 `ai.status` 显示“生成中”“不可用”或隐藏 AI 区域。
...
### GET /api/hot/ai（未来版，可选）

仅当未来需要独立 AI 请求、轮询、SSE 或更复杂实时补齐时再新增。

MVP 暂不实现该接口，统一通过 `/api/hot` 的 `ai.status` 表达 AI 状态。
...
说明：前端首次加载遇到 `ai.status="pending"` 时，显示“AI 内容生成中”，并在 30 秒后自动重新请求一次 `/api/hot`。
...
- 生成中：返回 `ai.available=false` + `ai.status="pending"`，前端显示轻量占位并 30 秒后重试一次。
- 失败：返回 `ai.available=false` + `ai.status="unavailable"`，前端隐藏 AI 区域或显示轻量占位。
...
- 本地开发：可默认关闭 AI；需要调试 AI 时手动刷新才调用，避免浪费额度。
- 生产 MVP 第二阶段：开启 AI，每 10 分钟最多调用一次。
...
const CATEGORY_IMAGES: Record<string, string> = {
  ...
  汽车: "/images/category/auto.jpg",       // MVP 第二阶段加入
...
MVP 第一阶段可先保留一个完整响应缓存，降低启动复杂度：

| 缓存对象 | TTL | Key | 说明 |
|----------|-----|-----|------|
| 完整响应 | 300 秒 | `hot:all` | 缓存完整 `/api/hot` 响应 |

PRD MVP 验收阶段需要拆分为单平台缓存和 AI 缓存：
...
1. 基础热榜已命中但 AI 未完成：返回 `ai.status="pending"`，后台继续生成 AI 结果。
2. AI 生成成功：写入 `ai:result`，下次 `/api/hot` 返回 `ai.status="ready"`。
3. AI 生成失败：写入短期失败状态，返回 `ai.status="unavailable"`，避免短时间内反复调用。
...
| AI 未开启 | `ai.available=false` + `ai.status=unavailable`，隐藏 AI 区域 |
| AI 生成中 | `ai.available=false` + `ai.status=pending`，AI 区域显示“AI 内容生成中”，首次加载 30 秒后自动重试一次 |
| AI 超时或格式错误 | 丢弃 AI 结果，返回 `ai.status=unavailable`，基础热榜正常返回 |
...
6. 实现单平台失败降级。
7. 实现综合热榜算法。
8. 拆分单平台缓存和 AI 缓存，完成 PRD MVP 的 8 个核心平台。
9. 接入 AI 标签、热门原因和推荐，并实现 `ai.status` 的 ready / pending / unavailable 状态。
