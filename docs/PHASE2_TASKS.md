...
- **视觉**：米白 riso 浅色主题（`docs/redesign/redesign-v1.html` 的 `data-theme="riso"`）；深色 + 主题切换器留第三阶段。
- **推进方式**：分步——先组 0 重构现有组件，再 D/E/F 写新组件，新组件一次用米白视觉写到位。
- **AI 区块**：组 0 阶段先占位/隐藏，沿用现有 `ai.status` 降级逻辑，等 B/C 做完再填真数据。
- **虎扑**：无稳定公开 JSON，做可降级平台（接入失败显示「数据源维护中」）。
- **部署**：临近评审时部署到香港服务器（免备案，国内可访问），开发期间先不动部署。

## 执行步骤（按开发顺序，每步构建+验证+提交）

| 步 | 内容 | 产出 |
|----|------|------|
| **0** | 改 `AGENTS.md` 过期横幅指向第二阶段；提交存档 `docs/redesign/` | 文档对齐、设计稿入库 |
| **1** | 组 0-1：从 riso 主题提取色板/字体/间距 → `variables.css` | 全站切米白视觉 |
| **2** | 组 0-2：重构 `Sidebar/TopBar/RankingList/PlatformCard/HotItem`（只改样式，不碰数据流） | 首页+平台页米白新设计 |
| **3** | 组 0-3/0-4：全局样式 + Loading/Empty/Error/Toast 对齐 + 还原比对 + 移动端 | 视觉改版收尾 |
| **4** | A：接入头条 → 澎湃 → 36氪（接口已调研），注册进聚合 | `/api/hot` 4 → 7 平台真实 |
| **5** | A：虎扑可降级 + 8 平台跑通 + 降级验证 | 8 平台体系完整 |
| **6** | B：AI 地基 `deepseek/prompts/parser` + AI 缓存 + 分类默认图 + `ai.status` 状态机（**需 DeepSeek API Key**） | AI 可调用，关闭时基础热榜照常 |
| **7** | C：C1 分类标签 → C2 今日最热+热点速览 → C3 个性推荐 → C4 路由整合 | `/api/hot` 返回完整 AI 数据 |
| **8** | D：今日最热轮播 + 热点速览轮播 + AI 降级态（pending 30s 重试），用新视觉 | 首页三段式完整 |
| **9** | E：个性推荐视图（标签筛选 + 推荐卡片 + localStorage 兴趣标签） | 个性推荐页可用 |
| **10** | F：平台视图 8 卡 + 每条 AI 标签 + 标签筛选栏 + 「汽车」标签 | 平台页带 AI 标签筛选 |
| **11** | G：端到端联调 + 降级演练 + 两端构建 + 更新 README | 可交付、可演示 |
| **12** | 部署到香港服务器（Nginx + PM2），改 `CLIENT_ORIGIN`/`VITE_API_BASE` | 国内可访问的线上版 |

> 顺序逻辑：组 0 视觉地基 → 后端 A/B/C 数据 → 前端 D/E/F 用新视觉接数据 → 收尾部署。
> 唯一需用户配合：步骤 6 的 DeepSeek API Key。
> 评审兜底：时间紧时保「步骤 0~5 + 步骤 7 的 C1 + 步骤 10 的 F2」= 米白视觉 + 多平台 + AI 标签。
